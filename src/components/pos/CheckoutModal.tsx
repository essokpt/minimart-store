...
import { X, CreditCard, Banknote, QrCode, CheckCircle2, Printer, ArrowRight, Loader2 } from 'lucide-react';
              ...
              >
                <div className="space-y-4 mb-8">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-5xl font-mono font-extrabold text-primary tracking-tight">${total.toFixed(2)}</p>
                  </div>
                </div>
                    ...
                    >
                      ...
                      {Number(receivedAmount) > 0 && Number(receivedAmount) >= total && (
                        <div className="mt-3 flex justify-between items-center text-sm font-medium">
                          ...
                          <span className="font-mono font-bold text-primary text-lg">${(Number(receivedAmount) - total).toFixed(2)}</span>
                        </div>
                      )}
                      {Number(receivedAmount) > 0 && Number(receivedAmount) < total && (
                  ...
                  <div className="w-full bg-white text-black p-6 rounded-md shadow-inner border border-outline-variant/30 text-left relative print-only font-mono text-sm h-64 overflow-y-auto print:h-auto print:overflow-visible my-2">
                    ...
                    <div className="space-y-2 mb-4">
                      {completedOrder.items.map(item => (
                        <div key={item.id} className="flex justify-between items-start text-xs border-b border-black/5 pb-2">
                    ...
                    <div className="pt-2 space-y-1">
                      <div className="flex justify-between text-xs">
                        <span>Subtotal</span>
                        <span>${(completedOrder.total / 1.08).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span>Tax (8%)</span>
                        <span>${(completedOrder.total - (completedOrder.total / 1.08)).toFixed(2)}</span>
                      </div>
                      ...
                      {completedOrder.method === 'cash' && (
                        <>
                          <div className="flex justify-between text-xs pt-1">
                            <span>Tendered</span>
                            <span>${completedOrder.received?.toFixed(2)}</span>
                          </div>
